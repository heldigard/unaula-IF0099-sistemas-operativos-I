# IF0099 - Sistemas Operativos I
## Portal del Curso - Plan Práctico v2.0 (2026-I)

**Universidad Autónoma Latinoamericana - UNAULA**  
**Facultad de Ingenierías**  
**Docente:** Ing. Eldigardo Evelio Camacho Cespedes

---

## 🚀 Acceso Rápido (Contenido Web)
Este curso está diseñado para ser visualizado en formato HTML. Utiliza los siguientes enlaces para navegar por el contenido oficial:

- 📅 **[Cronograma Detallado (Tue/Thu)](https://eldigardocamacho.github.io/IF0099-sistemas-operativos-I/clases-html/cronograma.html)**
- 📋 **[Rúbricas de Evaluación](https://eldigardocamacho.github.io/IF0099-sistemas-operativos-I/clases-html/rubricas.html)**
- 📖 **[Glosario de Términos](https://eldigardocamacho.github.io/IF0099-sistemas-operativos-I/clases-html/glosario.html)**
- 🧪 **[Índice de Laboratorios](https://eldigardocamacho.github.io/IF0099-sistemas-operativos-I/laboratorios-html/index.html)**

---

## 📚 Clases Teórico-Prácticas (50/50)

| # | Semana | Tema Principal | Enlace |
|---|--------|----------------|--------|
| 01 | Semana 1 | ¿Qué es un Sistema Operativo? | [Ver Clase 01](https://eldigardocamacho.github.io/IF0099-sistemas-operativos-I/clases-html/clase-01.html) |
| 02 | Semana 1 | Linux Terminal - Comandos Esenciales | [Ver Clase 02](https://eldigardocamacho.github.io/IF0099-sistemas-operativos-I/clases-html/clase-02.html) |
| 04 | Semana 2 | Estructura del Sistema Operativo | [Ver Clase 04](https://eldigardocamacho.github.io/IF0099-sistemas-operativos-I/clases-html/clase-04.html) |
| 05 | Semana 3 | Procesos en Linux - Gestión y Monitoreo | [Ver Clase 05](https://eldigardocamacho.github.io/IF0099-sistemas-operativos-I/clases-html/clase-05.html) |
| 06 | Semana 4 | Procesos en Windows - Gestión y Servicios | [Ver Clase 06](https://eldigardocamacho.github.io/IF0099-sistemas-operativos-I/clases-html/clase-06.html) |
| 07 | Semana 4 | Hilos y Sincronización | [Ver Clase 07](https://eldigardocamacho.github.io/IF0099-sistemas-operativos-I/clases-html/clase-07.html) |
| 08 | Semana 5 | Gestión de Memoria | [Ver Clase 08](https://eldigardocamacho.github.io/IF0099-sistemas-operativos-I/clases-html/clase-08.html) |
| 09 | Semana 6 | Paginación | [Ver Clase 09](https://eldigardocamacho.github.io/IF0099-sistemas-operativos-I/clases-html/clase-09.html) |
| 10 | Semana 6 | Memoria Virtual | [Ver Clase 10](https://eldigardocamacho.github.io/IF0099-sistemas-operativos-I/clases-html/clase-10.html) |
| 11 | Semana 7 | Discos y E/S | [Ver Clase 11](https://eldigardocamacho.github.io/IF0099-sistemas-operativos-I/clases-html/clase-11.html) |
| 12 | Semana 9 | Sistemas de Archivos | [Ver Clase 12](https://eldigardocamacho.github.io/IF0099-sistemas-operativos-I/clases-html/clase-12.html) |
| 13 | Semana 9 | Implementación de Sistemas de Archivos | [Ver Clase 13](https://eldigardocamacho.github.io/IF0099-sistemas-operativos-I/clases-html/clase-13.html) |
| 14 | Semana 10 | Servidor Web IIS | [Ver Clase 14](https://eldigardocamacho.github.io/IF0099-sistemas-operativos-I/clases-html/clase-14.html) |
| 15 | Semana 11 | Permisos y Seguridad en Linux | [Ver Clase 15](https://eldigardocamacho.github.io/IF0099-sistemas-operativos-I/clases-html/clase-15.html) |
| 16 | Semana 11 | Permisos y Seguridad en Windows | [Ver Clase 16](https://eldigardocamacho.github.io/IF0099-sistemas-operativos-I/clases-html/clase-16.html) |
| 17 | Semana 12 | Servidor Web Apache | [Ver Clase 17](https://eldigardocamacho.github.io/IF0099-sistemas-operativos-I/clases-html/clase-17.html) |

---

## 🧪 Laboratorios Prácticos

| Lab | Tema | Infraestructura |
|-----|------|-----------------|
| 00 | VirtualBox + Ubuntu Server | [Ver Lab 00](https://eldigardocamacho.github.io/IF0099-sistemas-operativos-I/laboratorios-html/lab-00-introduccion-simuladores.html) |
| 01 | Comandos Básicos Linux | [Ver Lab 01](https://eldigardocamacho.github.io/IF0099-sistemas-operativos-I/laboratorios-html/lab-01-comandos-basicos.html) |
| 02 | Permisos y Usuarios | [Ver Lab 02](https://eldigardocamacho.github.io/IF0099-sistemas-operativos-I/laboratorios-html/lab-02-permisos-usuarios.html) |
| 03 | Gestión de Procesos | [Ver Lab 03](https://eldigardocamacho.github.io/IF0099-sistemas-operativos-I/laboratorios-html/lab-03-gestion-procesos.html) |
| 04 | Memoria y Monitoreo | [Ver Lab 04](https://eldigardocamacho.github.io/IF0099-sistemas-operativos-I/laboratorios-html/lab-04-memoria-monitoreo.html) |
| 05 | Sistemas de Archivos | [Ver Lab 05](https://eldigardocamacho.github.io/IF0099-sistemas-operativos-I/laboratorios-html/lab-05-sistemas-archivos.html) |
| 06 | Seguridad y Hardening | [Ver Lab 06](https://eldigardocamacho.github.io/IF0099-sistemas-operativos-I/laboratorios-html/lab-06-seguridad-hardening.html) |
| AWS | Introducción a EC2 | [Ver Lab AWS](https://eldigardocamacho.github.io/IF0099-sistemas-operativos-I/laboratorios-html/lab-aws-01-introduccion-ec2.html) |

---

## 📄 Documentos Oficiales (GitHub Repo)
Los siguientes archivos deben ser visualizados o descargados directamente desde el repositorio:

- 📄 [Acta de Concertación (PDF)](https://github.com/EldigardoCamacho/IF0099-sistemas-operativos-I/blob/main/docs/AC-IF0099-001-2026-1-Sistemas%20Operativos%20I.pdf)
- 📅 [Calendario Festivos Colombia 2026](https://github.com/EldigardoCamacho/IF0099-sistemas-operativos-I/blob/main/docs/FestivosColombia2026.txt)
- ⛪ [Receso Semana Santa 2026](https://github.com/EldigardoCamacho/IF0099-sistemas-operativos-I/blob/main/docs/SemanaSanta2026.txt)

---

## 🛠️ Tecnologías Utilizadas
- **Virtualización:** VirtualBox 7.0
- **Linux:** Ubuntu Server 22.04 / 24.04 LTS
- **Windows:** Windows Server 2022
- **Cloud:** AWS EC2 (Free Tier)
- **Web:** Apache2, IIS

---
**Última actualización:** 16 de Febrero, 2026  
*Este portal es la fuente oficial de verdad para el semestre 2026-I.*
